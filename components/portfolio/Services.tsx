        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Reveal key={service.id} yOffset={30} delay={index * 0.1}>
              <div className="h-full flex flex-col justify-between border border-hairline bg-ink text-canvas p-8 rounded-2xl relative group overflow-hidden shadow-xl hover:border-accent/50 transition-all duration-300">
                <div>
                  {/* Card Number */}
                  ...
                  {/* Title & Description */}
                  <h3 className="text-2xl font-bold font-display text-canvas mb-4 uppercase">
                    {service.title}
                  </h3>
                ...
                <div className="pt-6 border-t border-white/10">
                  <a
                    href="#contact"
                    className="w-full inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-accent hover:text-on-accent text-canvas font-mono text-xs font-semibold uppercase py-3.5 px-4 rounded-full transition-all touch-target"
                  >

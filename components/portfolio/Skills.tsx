...
export function Skills({ skillGroups }: SkillsProps) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category.toLowerCase()) {
      case 'frontend':
        return <Code2 className="w-5 h-5 text-lime" />;
      case 'product':
        return <Layout className="w-5 h-5 text-lime" />;
      case 'backend & edge':
        return <Server className="w-5 h-5 text-lime" />;
      case 'workflow':
        return <Cpu className="w-5 h-5 text-lime" />;
      default:
        return <Code2 className="w-5 h-5 text-lime" />;
    }
  };
  ...
  return (
    <section id="skills" className="py-20 md:py-28 border-b border-hairline bg-paper">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {skillGroups.map((group, index) => (
            <Reveal key={group.category} yOffset={30} delay={index * 0.1}>
              <div className="h-full border border-hairline bg-white/70 hover:bg-white p-6 rounded-2xl transition-all duration-300 shadow-sm flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-3 border-b border-hairline pb-4 mb-4">
                    ...
                    <div>
                      <span className="font-mono text-[10px] uppercase text-slate block">
                  ...
                  <ul className="space-y-2.5">
                    {group.skills.map((skill) => (
                      <li
                        key={skill}
                        className="font-mono text-xs text-slate flex items-center gap-2"
                      >
                        <span className="w-1.5 h-1.5 rounded-full bg-lime border border-ink/20" />
                        <span>{skill}</span>
                      </li>
                    ))}
                  </ul>
